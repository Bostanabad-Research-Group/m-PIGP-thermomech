# =====================================================================
# EX2D1 — thermo-mechanical GRIPPER (four-phase {void, Ti, Cu, Steel}).
...
# The framework body is shared with run_actuator.py (EX2D2); only the
# example-specific parts differ:
#   * get_data import -> get_data_EX2D1_thermomech_gripper_four_phase_source
#   * Example/Case, design_flag = True (phase GP conditioning)
#   * adjoint fixed-dof function -> gripper BCs per paper Fig. 2(b):
#     LEFT and BOTTOM edges clamped, TOP edge roller up to the notch
#     step at x = 400; notch faces / jaw ledge / right edge free (see
#     _ex2d1_adjoint_fixed_dofs and GRIP_FREE_X0 below)
#   * uout_comp = 1: the output spring K_s is VERTICAL at the jaw tip,
#     so u_out (objective offset + best-design banking) reads the
#     v-component of the port node instead of EX2D2's horizontal u
...
from utils.get_training_data import get_data_EX2D1_thermomech_gripper_four_phase_source as get_data
...
def _find_adjoint_fixed_dofs(collocation_x, model_adj, MP):
    ...
    2. MP['adjoint_fixed_dofs_fn'] : callable(collocation_x) -> LongTensor.
                                     ...
                                     mesh slice; see the EX2D1 gripper default
                                     in the config block).
...
def calculate_TO_loss(model_list):
    ...
    # u_out COMPONENT: MP['uout_comp'] selects which displacement component
    # of the (first) output-spring node is the reported/banked stroke.
    # EX2D2 actuator: horizontal spring -> 0 (u, the default).  EX2D1
    # gripper: VERTICAL spring K_s at the jaw tip, u_out = +v (paper
    # Fig. 2(b)) -> 1.  The read also drives the best-design banking,
    # so it must be the true stroke component.
    _uc = MP.get('uout_comp', 0)
    if MP.get('maximize_uout', False):
        obj = - disp_K_out[0, _uc].detach() + mech_sens + heat - source
    ...
    loss_dict = {
        'obj_func': obj,
        'u_out': disp_K_out[0, _uc].detach(),  # actual output displacement (signed, NN-equilibrium estimate; component per MP['uout_comp'])
...
if __name__ == "__main__":
    ...
    design_flag = True    # gripper: phase GP conditioned on the phase{i}
                          #   training points (passive regions / ports)
    tau = 0.5
    ...
    K_out = 2e-3
    f_out = 0.1
    uout_comp = 1   # GRIPPER: output spring K_s is VERTICAL at the jaw tip
                    #   (paper Fig. 2(b)), so the reported/banked stroke is the
                    #   v-component of the port node.  (EX2D2 actuator uses 0.)
    a_T = 1.0
    nu = 0.31
    p0 = 1
    pf = 3
    ...
    # ---------------------------------------------------------------------
    # EX2D1 GRIPPER — from paper Fig. 2(b) (TM_TO_IDETC revised):
    #   * LEFT edge (x=xmin, carries the TD = 673 K line) CLAMPED
    #     (lambda_u = lambda_v = 0);
    #   * BOTTOM edge (y=ymin) CLAMPED (hatched, no rollers in the figure);
    #   * TOP edge (y=ymax) ROLLER (lambda_v = 0, lambda_u free) — the
    #     roller row ends at the notch step x = GRIP_FREE_X0;
    #   * the top-right 100x100 um NOTCH is cut out of the domain (its
    #     cells are dropped by the unigrid geometry mask), so the notch
    #     faces, the jaw ledge at y = 150 (solid 100 um jaw bar with the
    #     VERTICAL output spring K_s at its tip, u_out = +v) and the right
    #     edge are all FREE.
    # The adjoint satisfies the SAME homogeneous kinematic BCs as the
    # primal (paper Eq. 17a), so this replaces the
    # snap-to-conditioning-points fallback with the exact
    # primal-constrained space — eliminating over/under-pinning as a
    # failure mode.  The BC audit (set difference vs the primal GP
    # conditioning points, with coordinates) and the load-kill guard in
    # solve_adjoint_direct still validate this against get_data on the
    # first solve.  Alternative: comment out the
    # MP['adjoint_fixed_dofs_fn'] assignment below to fall back to
    # snapping the adjoint conditioning points (the get_data BC set).
    # =====================================================================
    GRIP_FREE_X0 = 400.0   # notch step: top-edge roller ends here (xmax minus
                           #   the 100 um jaw ledge; paper Fig. 2(b)).  Nodes at
                           #   y=ymax with x > this are dropped with the notch
                           #   cells anyway — the condition is a guard.
    def _ex2d1_adjoint_fixed_dofs(X):
        x, y = X[:, 0], X[:, 1]
        tolx = 1e-4 * (x.max() - x.min()).clamp_min(1e-30)
        toly = 1e-4 * (y.max() - y.min()).clamp_min(1e-30)
        idx = torch.arange(X.shape[0], device=X.device)
        clamped = idx[(x <= x.min() + tolx) | (y <= y.min() + toly)]  # left + bottom
        roller  = idx[(y >= y.max() - toly) & (x <= GRIP_FREE_X0)]    # top, to the notch
    ...
    MP['r_min'] = r_min
    MP['adjoint_fixed_dofs_fn'] = _ex2d1_adjoint_fixed_dofs
