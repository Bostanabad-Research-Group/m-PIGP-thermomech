    ...
    def forward(self, x, y):
        ...
        for alpha in self.F_active:
            ...
            F = grid_sample_2d(
                ...
                step=self.mode,
                offset=True,
                offset_jitter=self.offset_jitter if self.training else 0.0,
            )
            dim = alpha.shape[1]
...
def grid_sample_2d(input, grid, step = 'cosine', offset = True, offset_jitter = 0.0):
    '''
    Args:
    ...
    if offset: # predefine the offset can accelerate training
        base_offset = torch.linspace(0,1-(1/N),N,device=grid.device, dtype=grid.dtype).reshape(N,1,1)

        if offset_jitter > 0:
            if not (0.0 <= offset_jitter <= 1.0):
                raise ValueError("offset_jitter must satisfy 0 <= offset_jitter < 1")

            # Half of the adjacent-grid distance is 0.5 / N. Keep the random
            # perturbation to a user-chosen fraction of that bound.
            jitter_bound = offset_jitter * (0.5 / N)
            jitter_y = (2.0 * torch.rand((N, 1, 1), device=grid.device, dtype=grid.dtype) - 1.0) * jitter_bound
            jitter_x = (2.0 * torch.rand((N, 1, 1), device=grid.device, dtype=grid.dtype) - 1.0) * jitter_bound
        else:
            jitter_y = 0.0
            jitter_x = 0.0

        iy = ((iy+1)/2)*(IH-2) + base_offset + jitter_y
        ix = ((ix+1)/2)*(IW-2) + base_offset + jitter_x
...
class NetworkM4_fused_rho(nn.Module):
    ...
    def forward(self, input, features):
        ...
        out = torch.sigmoid(self.last(H))
        # out = torch.tanh(self.last(H))
        return out
